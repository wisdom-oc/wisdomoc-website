    ...
    function autoPlay() {
      if(!isAutoPlaying) {
        intervalId = setInterval(() => {
          ...
          playGame(playerMove);
        }, 1000); /* computer will keep on running the function every 1s */
        isAutoPlaying = true;
      } else {
        clearInterval(intervalId);
        isAutoPlaying = false;
      } 
    } // clearInterval() helps to stop the setInterval when clicked
...
/*Gets the button from HTML and adds it to JavaScript, then use document.querySelector to select it and add eventListener to it*/ 
...
// .addEventListener also provide the event but as parameter in the function and event contains the key we press
